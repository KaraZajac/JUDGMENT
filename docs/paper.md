...
**Prediction.** Ruger et al. (2004) showed simple classification trees beating a panel of experts on the 2002 term (75% vs 59% case accuracy). Katz, Bommarito & Blackman (2017) built time-evolving random forests over SCDB features for 1816–2015, reporting 70.2% case / 71.9% justice-vote accuracy out-of-sample; theirs is the benchmark general model, but two regime differences make the comparison indicative rather than head-to-head: their feature discipline is decision-date knowability rather than cert-stage (features include whether oral argument was heard, whether a rehearing occurred, and the elapsed time from argument to decision), and their margin over the always-petitioner rule is roughly two points (Medvedeva, Wieling & Vols 2023 characterize it as "a small improvement over the 68% baseline accuracy where the petitioner always wins"; their own strongest null reaches 67.5%). Kaufman, Kraft & Sen (2019) report the highest published academic figure — 74.04% case accuracy over 2005–2015 — but the entire above-baseline margin comes from oral-argument transcript features: their case-covariates-only model scores 60.6%, *below* the 67.98% always-petitioner baseline, under pooled ten-fold cross-validation rather than walk-forward evaluation. Read against our results, that finding independently corroborates this paper's central negative result — at the cert stage, structure is nearly all there is, and predictive content arrives with argument — and it motivates the staged registration design of §6.

**Crowds and markets.** The FantasySCOTUS crowd is the one system that genuinely outperforms our numbers, and it does so on later information: over OT2011–2016 (425 cases, 7,284 participants) fair aggregation rules score ~74.1% case / ~73.5% justice-vote (the widely quoted 80.8%/77.1% is the authors' own post-hoc ceiling across 277,201 aggregation configurations, not a deployable forecast), with predictions revisable until the eve of decision — embedding briefing, oral argument, and news coverage unavailable at cert — and no probabilities, Brier, or calibration reported (Katz, Bommarito & Blackman 2017b). The platform's self-scored 2025-term crowd reached 94.7% case / 91.4% per-vote — above the ~83% per-vote ceiling of an oracle that knows every outcome and predicts all-majority votes, i.e., the crowd genuinely anticipates dissent structure, a benchmark our per-justice probabilities can be scored against prospectively. Prediction markets provide no systematic docket coverage (merits-outcome markets appear episodically for exceptionally salient cases). No verified LLM-based system reports accuracy above ours (per-justice GPT-2 ensembles: 60% case, Hamilton 2023; the JUSTICE text benchmark: ≤68% despite augmented case variants straddling a random train/test split), and Medvedeva, Wieling & Vols (2023) document that much of the wider "legal judgment prediction" literature classifies post-decision documents rather than forecasting from pre-decision information. As of mid-2026, no algorithmic system publishes live, calibrated, preregistered forecasts of pending cases — {Marshall}+, the crowd's algorithmic counterpart, is dormant — so the registry of §6 occupies an empty lane.
...
Every granted-but-undecided case receives a forecast: calibrated P(reverse), per-justice probabilities for both targets, and the full coalition split distribution, each file carrying model version, feature-availability notes, and hand-coding bases. Pending-case forecasts are re-issued at each refresh as hand-codings improve; every vintage is timestamped in the public git history, and the scorer enforces the preregistration property directly rather than by convention: a forecast whose file was (re)generated on or after its case's decision date is excluded from the track record as *not ex-ante*, never scored. Post-hoc revision is thus both detectable and inert. Registration is staged: the cert-stage forecast freezes on argument day — once a case is argued its file is never regenerated, so the registered cert-stage record cannot silently absorb argument-informed recoding or newer model vintages. A post-argument second stage, re-registered under its own timestamp and scored separately (reserved at `data/forecasts/<term>/post-argument/`), deploys only when argument-derived features clear the same walk-forward gate as every deployed configuration (§4.6). They now have: per-justice questioning features parsed from Oyez's structured transcripts (7,023 argued cases, 96% of 1955–2024) add +1.7 points of per-vote accuracy on transcript-covered rows (McNemar p ≈ 2×10⁻¹⁵), with an instructive era structure — +3.8 to +4.7 points sustained across the 1980s–2010s (the per-justice, walk-forward counterpart of Kaufman, Kraft & Sen's case-level result), negative in the 1950s–60s (heuristic side attribution, cold benches), and initially ~zero in the 2020s, where the post-2020 seriatim argument format mechanically compresses turn-count asymmetries. Two refinements resolve that caveat (docs/postargument-gate.md): word-centric questioning features with a within-justice lagged baseline (format and style norms cancel), and the side the Solicitor General supports as amicus at argument — on SG-covered rows the strongest single feature we have tested (+5.2 points; the SG-supported side reverses 75.8% vs 42.3%, McNemar p ≈ 8×10⁻¹⁷). The deployed post-argument configuration (reverse: 69.6% per-vote, Brier 0.1997, AUC 0.723) lifts the 2020s covered slice from 68.6% to 72.5% — the current Court's format becomes the model's best modern era rather than its weakest — and its live track record adjudicates prospectively. As decisions land (via the interim and syllabus pipelines, with consolidated companions resolved to their deciding record per §3), a scorer resolves outcomes (SCDB disposition tokens when available; a flagged winning-party proxy for provisional records), computes case- and justice-level hits and Brier, and publishes a running track record. At this writing the registry holds 17 forecasts, 16 awaiting decision (every case leaning reverse, reflecting certiorari selection's ~70% recent-term reversal base rate). The registry's first resolution exercised the exclusion rule rather than the scorer: *Little v. Hecox* was decided as a consolidated companion of *West Virginia v. B.P.J.* days before its forecast's final regeneration, so it enters the record as not ex-ante — excluded even though it would have scored as a hit. The system self-updates: daily decision-day pipeline (interim ingest, syllabus vote extraction, scoring), daily lower-court-opinion harvesting, weekly full refresh — all committed by CI with substantive-change detection.
...
Three further criticisms were converted into experiments (docs/methods-audit.md): a **nested-selection replay** — every feature adoption re-decided on 1956–2010 evidence alone reproduces the deployed configuration chain exactly, and the chosen model scores 71.5% per-vote (Brier 0.190) on the untouched 2011–2024 window, so the reuse of the validation window for model selection did not manufacture the headline; **case-clustered bootstrap inference** — every accuracy-based adoption survives clustering (lc +3.45pp [3.02, 3.86]; oral-argument features +1.73pp [1.11, 2.33]; SG-amicus +5.19pp [3.32, 7.10] on covered rows), with issue3t's accuracy null matching its probability-quality adoption basis; and a **blind validation of the pre-1970s argument-order heuristic** against 527 description-era cases — 99.0% section agreement on the two-advocate structure that dominates the era it serves, with errors concentrated in multi-advocate arguments, quantifying the early-era noise in the questioning features. A standing pre-commitment: when SCDB's release covers the forecast terms, we publish the agreement rate between our cert-stage hand-codes and SCDB's official coding, whatever it is.
...
Hamilton, S. (2023). Blind judgement: Agent-based Supreme Court modelling with GPT. arXiv:2301.05327.
...
Katz, D. M., Bommarito, M. J., & Blackman, J. (2017b). Crowdsourcing accurately and robustly predicts Supreme Court decisions. arXiv:1712.03846.
